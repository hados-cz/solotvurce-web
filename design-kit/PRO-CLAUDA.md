...
- **Živě:**
  - vizuální reference → https://solotvurci.cz/design-kit/styleguide
  - stylesheet → https://solotvurci.cz/design-kit/design.css
...
## Sync (důležité)
`design.css` je **extrahovaný z živého `solotvurci.cz`** (inline `<style>` v `index.html`).
